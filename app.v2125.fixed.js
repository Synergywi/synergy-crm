...
document.addEventListener('DOMContentLoaded', ()=>{ try{ const raw=localStorage.getItem('synergy_me'); if(raw){ const me=JSON.parse(raw); if(me&&me.email){ DATA.me=me; } } }catch(_){ }
  ...
  // Baseline Integrity Guard (keep silent if brand/primary missing)
  try{
    const reqVars=["--bg","--ink","--accent"];
...
// App state for calendar (default to ALL users for Admins)
...
function Calendar(){
  ...
  const grid = weeks.map(week=>{
    const tds = week.map(d=>{
      ...
      const chips = evs.map(e=>`<div class="cal-ev type-${(e.type||'').toLowerCase()}" title="${e.title} (${new Date(e.startISO).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})})">
...
/* Calendar actions + helpers */
function renderEventModal(ev){
  const overlay = document.createElement('div');
  overlay.style.cssText = "position:fixed;inset:0;background:rgba(2,6,23,.45);display:flex;align-items:center;justify-content:center;z-index:9999";
  const card = document.createElement('div');
  card.className = "card";
  card.style.cssText = "max-width:560px;width:92%;padding:16px";
  card.innerHTML = `
    <h3 class="section-title" style="margin-bottom:8px">${esc(ev.title||'Event')}</h3>
    <div class="muted" style="margin-bottom:8px">
      ${new Date(ev.startISO).toLocaleString()} – ${new Date(ev.endISO).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}
    </div>
    <div><strong>Owner:</strong> ${esc(ev.ownerName||ev.ownerEmail||'')}</div>
    <div><strong>Location:</strong> ${esc(ev.location||'—')}</div>
    ${ev.caseId ? `<div><strong>Linked case:</strong> ${esc(ev.caseId)}</div>` : ''}
    <div class="right" style="margin-top:12px">
      <button class="btn light" data-x="close">Close</button>
      <button class="btn danger" data-act="deleteEvent" data-arg="${ev.id}">Delete</button>
    </div>`;
  overlay.appendChild(card);
  overlay.addEventListener('click', (e)=>{ if(e.target.dataset.x==='close' || e.target===overlay){ overlay.remove(); } });
...
document.addEventListener('click', e=>{
  ...
  if(act==='createCaseEvent'){
    ...
    const sISO = date+'T'+start+':00';
    const eISO = date+'T'+end+':00';
    (DATA.calendar||(DATA.calendar=[])).push({ id:uid(), title, description:"", startISO:sISO, endISO:eISO, ownerEmail:owner, ownerName, location:loc, type, caseId });
    alert('Case event added'); App.set({}); return;
  }
  if(act==='deleteEvent'){
