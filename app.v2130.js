...
function CasePage(id){
  ...
  return Shell('<div class="card">Case not found.</div>','cases'); }
  const tab=App.state.tabs.case||'details';
  const invOpts=()=>DATA.users.filter(u=>["Investigator","Reviewer","Admin"].includes(u.role)).map(u=>`<option ${u.email===cs.investigatorEmail?'selected':''} value="${u.email}">${u.name} (${u.role})</option>`).join("");
  const coOpts=()=>DATA.companies.map(co=>`<option ${co.id===cs.companyId?'selected':''} value="${co.id}">${co.name} (${co.id})</option>`).join("");
...
function renderEventModal(ev){
  ...
  const myCases = (DATA.cases||[]).filter(cs=>{ const inv=(cs.investigatorEmail||cs.investigator||'')+''; return isAdmin || inv===me.email || inv===me.name; });
  const caseSelect = `<div><label>Case (optional)</label><select class="input" id="em-case"><option value="">—</option>${myCases.map(cs=>`<option value="${cs.id}" ${(cs.id===(ev.caseId||'')?'selected':'')}>${cs.fileNumber} — ${esc(cs.title||'')}</option>`).join('')}</select></div>`;
...
document.addEventListener('click', e=>{
  ...
  if(act==='createCaseEvent'){
    const csId=arg; const me=DATA.me||{email:"",name:""};
    ...
    DATA.calendar.push(ev); App.set({}); return;
  }
  if(act==='openEvent'){
