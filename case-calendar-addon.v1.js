...
(function(){
  ...
  document.addEventListener('click', function(e){
    ...
    if(!t||t===document) return; const act=t.getAttribute('data-act'), arg=t.getAttribute('data-arg'); ensureStores();
    ...
    if(act==='saveEvent'){ const ev=(DATA.calendar||[]).find(x=>x.id===arg); if(!ev) return;
      ...
      notify('updated', ev); persist(); document.getElementById('modal-root')?.remove(); if(typeof render==='function') render(); return; }

    if(act==='deleteEvent'){ const ev=(DATA.calendar||[]).find(x=>x.id===arg); if(ev) notify('deleted', ev);
      DATA.calendar=(DATA.calendar||[]).filter(x=>x.id!==arg); persist(); document.getElementById('modal-root')?.remove(); if(typeof render==='function') render(); return; }
    ...
    if(act==='createCaseEvent'){ const csId=arg; const me=DATA.me||{email:'',name:''};
      ...
      DATA.calendar.push(ev); notify('created', ev); persist(); if(typeof render==='function') render(); return; }

    if(act==='notifOpen'){ const n=(DATA.notifications||[]).find(x=>x.id===arg); if(!n) return; n.read=true; const ev=(DATA.calendar||[]).find(e=>e.id===n.evId); if(ev) renderEventModal(ev); persist(); if(typeof render==='function') render(); return; }
    if(act==='notifDismiss'){ const n=(DATA.notifications||[]).find(x=>x.id===arg); if(n) n.read=true; persist(); if(typeof render==='function') render(); return; }
    if(act==='notifToggle'){ const s=App.state.notifs||{show:'unread'}; s.show=(s.show==='all'?'unread':'all'); App.set({notifs:s}); return; }
    if(act==='notifMarkAll'){ for(const n of (DATA.notifications||[])) n.read=true; persist(); App.set({}); return; }
  }, true);
  ...
  function injectCaseCalendar(){
    ...
    const ownerSelect = (((DATA.me||{}).role||'')==='Admin')
      ...
      : "";
    const html = `
      <div class="tabpanel ${((App.state.tabs||{}).case==='calendar')?'active':''}" id="case-cal-ext">
  ...
  function injectDashboardNotifs(){
    if(!window.App || App.state.route!=='dashboard') return;
  ...
  function wrapSet(){
    if(!window.App || typeof App.set!=='function' || App.__caseCalWrapped) return;
    const _set=App.set.bind(App);
    App.set=function(p){ const r=_set(p); try{ injectCaseCalendar(); injectDashboardNotifs(); }catch(e){} return r; };
    App.__caseCalWrapped=true;
  }

  document.addEventListener('DOMContentLoaded', function(){ ensureStores(); wrapSet(); try{ injectCaseCalendar(); injectDashboardNotifs(); }catch(e){} });
})();
