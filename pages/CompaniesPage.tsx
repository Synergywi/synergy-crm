...
type Company = { id: number | string; name: string; website?: string | null; createdAt?: string };
...
async function createCompany(input: { name: string; website?: string }) {
      ...
      <div className="card span-8">
        ...
        {loading ? <div>Loading…</div> : err ? <div style={{color:"#ef4444"}}>Error: {err}</div> : (
          <table className="table">
            <thead><tr><th>ID</th><th>Name</th><th>Website</th><th>Created</th></tr></thead>
            <tbody>
              {data.map(c => (
                <tr key={c.id}>
                  <td>{c.id}</td>
...
function Create({ onCreated }: { onCreated: (c: Company) => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const created = await createCompany({ name: name.trim(), website: website.trim() || undefined });
      onCreated(created);
      setName(""); setWebsite("");
    } catch (e: any) {
