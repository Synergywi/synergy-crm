(function(){ "use strict";
const BUILD="v2.14.3"; const STAMP=(new Date()).toISOString();
console.log("Synergy CRM "+BUILD+" • "+STAMP);

// ---- Demo data (local, in-memory) ----
...
const DATA={
  ...
  notifications:[],   // Admin-only demo notifications bucket
...
const App={state:{route:"dashboard",currentCaseId:null,currentContactId:null,currentCompanyId:null,asUser:null,casesFilter:{q:""}}, set(p){Object.assign(App.state,p||{}); render();}, get(){return DATA;}};

// --- UI bits ---
function BellHTML(){
  const isAdmin=(DATA.me && DATA.me.role)==="Admin";
  const unread = (DATA.notifications||[]).filter(n=>!n.read).length;
  const cls = "bell"+(unread && isAdmin ? " has-unread": "");
  return isAdmin ? `<div class="${cls}" title="Notifications" data-act="openBell">
...
function Topbar(){return `<div class="topbar">
  ...
  <div class="sp"></div>
  ${App.state.asUser?`<span class="badge">You: ${(App.state.asUser.name||App.state.asUser.email)} (${App.state.asUser.role})</span>`:""}
  ${BellHTML()}
...
function Dashboard(){
  ...
  const notifRows = (DATA.notifications||[])
     ...
     .map(n=>`<tr><td>${n.time||""}</td><td>${n.action||""}</td><td>${n.title||""}</td><td class="right"><button class="btn light" data-act="dismissNotif" data-arg="${n.id}">Dismiss</button></td></tr>`)
     .join("") || '<tr><td colspan="4" class="muted">No notifications</td></tr>';

  const notifCard = isAdmin ?
    `<div class="card"><h3>Calendar updates (Admin)</h3>
      <div class="right" style="margin:6px 0 8px">
        <button class="btn light" data-act="showUnread">Show unread</button>
...
function Cases(){
  ...
  for(const cc of list){
    rows+=`<tr><td>${cc.fileNumber}</td><td>${cc.title}</td><td>${cc.organisation}</td><td>${cc.investigatorName}</td><td>${cc.status}</td><td class="right"><button class="btn light" data-act="openCase" data-arg="${cc.id}">Open</button></td></tr>`;
  }
  const tools=`<div class="grid cols-2" style="gap:8px"><input class="input" id="flt-q" placeholder="Search title, org, ID" value="${(f.q||'')}"></div>
...
function CasePage(id){
  ...
  const blocks=`<div class="grid cols-2">
    ...
    <div class="grid cols-2"><input class="input" id="task-title" placeholder="Task title"><input class="input" id="task-due" type="date"></div>
    <div class="grid cols-2"><select class="input" id="task-assignee">${invOpts()}</select><div class="right"><button class="btn light" data-act="addTask" data-arg="${id}">Add</button></div></div>
...
function CompanyPage(id){const d=App.get(), co=findCompany(id); if(!co) return Shell('<div class="card">Company not found.</div>','companies'); const contacts=d.contacts.filter(c=>c.companyId===id); const cases=d.cases.filter(c=>c.companyId===id); let crow=''; for(const c of contacts){crow+=`<tr><td>${c.name}</td><td>${c.email}</td><td class="right"><button class="btn light" data-act="openContact" data-arg="${c.id}">Open</button></td></tr>`;} let caserows=''; for(const k of cases){caserows+=`<tr><td>${k.fileNumber}</td><td>${k.title}</td><td>${k.status}</td><td class="right"><button class="btn light" data-act="openCase" data-arg="${k.id}">Open</button></td></tr>`;} return Shell('<div class="card"><div style="display:flex;align-items:center;gap:8px"><h2>'+co.name+'</h2><div class="sp"></div><button class="btn light" data-act="route" data-arg="companies">Back</button></div></div><div class="grid cols-2"><div class="section"><header><h3 class="section-title">Company Contacts</h3></header><table><thead><tr><th>Name</th><th>Email</th><th></th></tr></thead><tbody>'+crow+'</tbody></table></div><div class="section"><header><h3 class="section-title">Recent Cases</h3></header><table><thead><tr><th>Case</th><th>Title</th><th>Status</th><th></th></tr></thead><tbody>'+caserows+'</tbody></table></div></div>','companies');}
...
// notifications
if(act==='openBell'){App.set({route:'dashboard'}); return;}
if(act==='dismissNotif'){DATA.notifications = (DATA.notifications||[]).map(n=> n.id===arg ? Object.assign({}, n, {read:true}) : n); App.set({}); return;}
if(act==='showUnread'){DATA.notifications = (DATA.notifications||[]).map(n=> Object.assign({}, n, {read: !!n.read && n.read})); App.set({}); return;}
if(act==='markAllRead'){DATA.notifications = (DATA.notifications||[]).map(n=> Object.assign({}, n, {read:true})); App.set({}); return;}
...
document.addEventListener('change',e=>{
  ...
  // file pickers (docs/resources)
  if(e.target && e.target.id==='file-input'){
    const t = (App.state.currentUploadTarget||"").split("::");
    const csId = t[0], folder = t[1]||"General";
    const cs = findCase(csId);
    if(!cs) return;
    cs.folders[folder] = cs.folders[folder] || [];
    const files = Array.from(e.target.files||[]);
    files.forEach(f=>{
      const reader = new FileReader();
      reader.onload = function(ev){ cs.folders[folder].push({name:f.name,size:f.size,dataUrl:ev.target.result}); App.set({}); };
      reader.readAsDataURL(f);
    });
  }
  if(e.target && (e.target.id==='rs-file-templates' || e.target.id==='rs-file-procedures')){
    const bucket = e.target.id==='rs-file-templates' ? DATA.resources.templates : DATA.resources.procedures;
    const files = Array.from(e.target.files||[]);
    files.forEach(f=>{ const reader = new FileReader(); reader.onload=function(ev){ bucket.push({name:f.name,size:f.size,dataUrl:ev.target.result}); App.set({}); }; reader.readAsDataURL(f); });
  }
});

document.addEventListener('DOMContentLoaded',()=>{
  try{ const f=localStorage.getItem('synergy_filters_cases_v2104'); if(f) App.state.casesFilter=JSON.parse(f)||App.state.casesFilter; }catch(_){ }
