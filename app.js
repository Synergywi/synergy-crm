...
/* === Calendar v2: augment baseline without changing look === */
(function(){
  // seed structures
  if(!Array.isArray(DATA.calendar)) DATA.calendar = [];
  if(!Array.isArray(DATA.notifications)) DATA.notifications = [];
  // utility
  function _uid(){ return "ev-"+Math.random().toString(36).slice(2,10); }
  function _notify(action, ev){
    DATA.notifications.unshift({id:_uid(), when:(new Date()).toISOString(), action, title:ev.title, evId:ev.id, read:false});
  }
  // modal
  function _closeModal(){ const m=document.getElementById('ev-modal'); if(m) m.remove(); }
  function _openEventModal(ev){
    const root=document.createElement('div'); root.id='ev-modal';
    Object.assign(root.style,{position:'fixed',inset:'0',background:'rgba(0,0,0,.25)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:'1000'});
    root.innerHTML = `<div class="card" style="width:720px;max-width:94%">
      ...
      <div class="grid cols-2">
        ...
        <div><label>Start</label><input class="input" id="em-start" type="time" value="${(ev.startISO||'').slice(11,16)}"></div>
        <div><label>End</label><input class="input" id="em-end" type="time" value="${(ev.endISO||'').slice(11,16)}"></div>
        <div><label>Case (optional)</label><select class="input" id="em-case"><option value="">—</option>${
          (DATA.cases||[]).map(c=>`<option value="${c.id}" ${(ev.caseId===c.id)?'selected':''}>${esc(c.fileNumber)} — ${esc(c.title)}</option>`).join('')
        }</select></div>
      </div>
      <div class="right" style="margin-top:8px">
        <button class="btn" id="em-save">Save</button>
        <button class="btn danger" id="em-del">Delete</button>
        <button class="btn light" id="em-can">Cancel</button>
      </div>
    </div>`;
    root.addEventListener('click', (e)=>{ if(e.target.id==='ev-modal'||e.target.id==='em-can') _closeModal(); });
    document.body.appendChild(root);
    root.querySelector('#em-save').onclick = function(){
      const date=root.querySelector('#em-date').value||(ev.startISO||'').slice(0,10);
      const s=root.querySelector('#em-start').value||(ev.startISO||'').slice(11,16);
      const e=root.querySelector('#em-end').value||(ev.endISO||'').slice(11,16);
      ev.title=root.querySelector('#em-title').value||ev.title;
      ev.type=root.querySelector('#em-type').value||ev.type;
      ev.location=root.querySelector('#em-loc').value||ev.location;
      ev.caseId=(root.querySelector('#em-case').value||'')||null;
      ev.startISO = date+'T'+s+':00';
      ev.endISO = date+'T'+e+':00';
      _notify('updated',ev);
      App.set({}); _closeModal();
    };
    root.querySelector('#em-del').onclick=function(){
  ...
  // override Calendar renderer to add case linking + owner filter + open modal
  Calendar = function(){
    const me=DATA.me||{}; const isAdmin=(me.role==='Admin');
    const ownerFilter = (App.state.calendar&&App.state.calendar.filterUsers) || (isAdmin?'ALL':me.email);
    const events = (DATA.calendar||[]).filter(ev => ownerFilter==='ALL' || ev.ownerEmail===ownerFilter)
      .sort((a,b)=> String(a.startISO).localeCompare(String(b.startISO)));
    const rows = events.map(ev=>`<tr>
        <td>${new Date(ev.startISO).toLocaleDateString()}</td>
        <td>${(ev.startISO||'').slice(11,16)}–${(ev.endISO||'').slice(11,16)}</td>
        <td>${esc(ev.title)}</td>
        <td>${esc(ev.location||'')}</td>
        <td>${esc((ev.ownerName||ev.ownerEmail||''))}</td>
        <td>${esc(((DATA.cases||[]).find(c=>c.id===ev.caseId)||{}).fileNumber||'')}</td>
        <td class="right"><button class="btn light" data-act="openEvent" data-arg="${ev.id}">Open</button></td>
      </tr>`).join('') || `<tr><td colspan="7" class="muted">No events.</td></tr>`;
    const owners = isAdmin? `<select id="cal-owner-filter" class="input" style="max-width:220px"><option value="ALL"${ownerFilter==='ALL'?' selected':''}>All users</option>${
        (DATA.users||[]).map(u=>`<option value="${u.email}"${ownerFilter===u.email?' selected':''}>${esc(u.name)}</option>`).join('')
      }</select>`: `<span class="muted">${esc(me.name)} (you)</span>`;
    const caseCtl = `<select class="input" id="ev-case-opt"><option value="">—</option>${
      (DATA.cases||[]).map(c=>`<option value="${c.id}">${esc(c.fileNumber)} — ${esc(c.title)}</option>`).join('')
    }</select>`;
    return Shell(`<div class="card"><h3 class="section-title">Calendar</h3>
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
        <div class="muted">Agenda</div><div>${owners}</div>
      </div>
    ...
    <div class="card">
      ...
      <div class="grid cols-4">
        <input id="ev-title" class="input" placeholder="Title">
        <input id="ev-date" class="input" type="date" value="${(new Date()).toISOString().slice(0,10)}">
        <select id="ev-type" class="input"><option>Appointment</option><option>Note</option></select>
        <input id="ev-loc" class="input" placeholder="Location">
        <input id="ev-start" class="input" type="time" value="09:00">
        <input id="ev-end" class="input" type="time" value="10:00">
        ${caseCtl}
        ${isAdmin? `<select id="ev-owner" class="input">${(DATA.users||[]).map(u=>`<option value="${u.email}">${esc(u.name)} (${u.role})</option>`).join('')}</select>`:''}
      </div>
      <div class="right" style="margin-top:6px"><button class="btn" data-act="createEvent">Add</button></div>
    </div>`, 'calendar');
  };

  // Extend Dashboard to show notifications if present (without altering style)
  const _origDashboard = Dashboard;
  Dashboard = function(){
    const html = _origDashboard();
    const unread = (DATA.notifications||[]).filter(n=>!n.read);
    const list = (App.state && App.state.showUnread? unread : (DATA.notifications||[])).slice(0,12);
    const rows = list.map(n=>`<tr><td>${new Date(n.when).toLocaleString()}</td><td>${esc(n.action)}</td><td>${esc(n.title)}</td>
      <td class="right"><button class="btn light" data-act="notifOpen" data-arg="${n.id}">Open</button> <button class="btn" data-act="notifDismiss" data-arg="${n.id}">Dismiss</button></td></tr>`).join('') || `<tr><td colspan="4" class="muted">No notifications</td></tr>`;
    const block = `<div class="section"><header><h3 class="section-title">Calendar updates (Admin)</h3></header>
      <table><thead><tr><th>When</th><th>Action</th><th>Title</th><th></th></tr></thead><tbody>${rows}</tbody></table>
      <div class="right" style="margin-top:6px"><button class="btn light" data-act="notifToggle">${(App.state && App.state.showUnread)?'Show all':'Show unread'}</button>
      <button class="btn light" data-act="notifMarkAll">Mark all read</button></div></div>`;
    return html.replace('</div></div>', block + '</div></div>');
  };

  // Hook into global click handler by delegation (reuse existing handler if present)
  document.addEventListener('click', function(e){
    ...
    if(act==='createEvent'){
      ...
      const type=document.getElementById('ev-type').value||'Appointment';
      const loc=document.getElementById('ev-loc').value||'';
      const ownerEmail = isAdmin ? (document.getElementById('ev-owner')?.value || me.email) : me.email;
      const ownerName = ((DATA.users||[]).find(u=>u.email===ownerEmail)||{}).name || ownerEmail;
      const caseId = document.getElementById('ev-case-opt')?.value || null;
      const ev={id:_uid(), title, type, location:loc, startISO:date+'T'+start+':00', endISO:date+'T'+end+':00', ownerEmail, ownerName, caseId};
      DATA.calendar.push(ev); _notify('created', ev); App.set({}); return;
    }
    if(act==='openEvent'){ const ev=(DATA.calendar||[]).find(x=>x.id===arg); if(ev) _openEventModal(ev); return; }
    if(act==='notifToggle'){ App.set({showUnread: !(App.state && App.state.showUnread)}); return; }
    if(act==='notifMarkAll'){ (DATA.notifications||[]).forEach(n=>n.read=true); App.set({}); return; }
    if(act==='notifOpen'){ const n=(DATA.notifications||[]).find(n=>n.id===arg); if(n){ n.read=true; const ev=(DATA.calendar||[]).find(x=>x.id===n.evId); if(ev) _openEventModal(ev); App.set({}); } return; }
    if(act==='notifDismiss'){ DATA.notifications = (DATA.notifications||[]).filter(n=>n.id!==arg); App.set({}); return; }
    if(act==='createCaseEvent'){
      const csId = arg;
      const wrap = document.getElementById('case-cal-wrap'); if(!wrap) return;
      const title=wrap.querySelector('#cc-title').value.trim()||'Untitled';
      const date=wrap.querySelector('#cc-date').value;
      const start=wrap.querySelector('#cc-start').value||'10:00';
      const end=wrap.querySelector('#cc-end').value||'11:00';
      const type=wrap.querySelector('#cc-type').value||'Appointment';
      const loc=wrap.querySelector('#cc-loc').value||'';
      const ownerEmail = isAdmin? (wrap.querySelector('#cc-owner')?.value || me.email) : me.email;
      const ownerName = ((DATA.users||[]).find(u=>u.email===ownerEmail)||{}).name || ownerEmail;
      const ev={id:_uid(), title, type, location:loc, startISO:date+'T'+start+':00', endISO:date+'T'+end+':00', ownerEmail, ownerName, caseId:csId};
      DATA.calendar.push(ev); _notify('created', ev); App.set({}); return;
    }
  }, true);

  // Inject a Case Calendar section after case page renders
  const _origRender = render;
  render = function(){
    _origRender();
    if(App.state.route==='case'){
      const host = document.getElementById('app');
      if(!host) return;
      let wrap = document.getElementById('case-cal-wrap');
      if(wrap) wrap.remove();
      ...
      const rows=list.map(e=>`<tr><td>${new Date(e.startISO).toLocaleDateString()}</td><td>${(e.startISO||'').slice(11,16)}–${(e.endISO||'').slice(11,16)}</td><td>${esc(e.title)}</td><td>${esc(e.location||'')}</td><td>${esc(e.ownerName||e.ownerEmail||'')}</td><td class="right"><button class="btn light" data-act="openEvent" data-arg="${e.id}">Open</button></td></tr>`).join('') || `<tr><td colspan="6" class="muted">No events yet.</td></tr>`;
      const me=DATA.me||{}; const isAdmin=(me.role==='Admin');
      const ownerCtl = isAdmin? `<select class="input" id="cc-owner">${(DATA.users||[]).map(u=>`<option value="${u.email}">${esc(u.name)}</option>`).join('')}</select>`:'';
      const card = document.createElement('div');
      card.id='case-cal-wrap';
      card.innerHTML = `<div class="section"><div class="card"><h3 class="section-title">Calendar</h3>
      ...
      <div class="card"><h3 class="section-title">Add case event</h3>
        <div class="grid cols-4">
          <input class="input" id="cc-title" placeholder="Title">
          <input class="input" id="cc-date" type="date" value="${(new Date()).toISOString().slice(0,10)}">
          <select class="input" id="cc-type"><option>Appointment</option><option>Note</option></select>
          <input class="input" id="cc-loc" placeholder="Location">
          <input class="input" id="cc-start" type="time" value="10:00">
          <input class="input" id="cc-end" type="time" value="11:00">
          ${ownerCtl}
        </div>
        <div class="right" style="margin-top:6px"><button class="btn" data-act="createCaseEvent" data-arg="${csId}">Add</button></div>
      </div></div>`;
