...
document.addEventListener('click', e=>{
  ...
  if(act==='createEvent'){
    ...
    const ev={id:uid(), title, description:"", startISO:sISO, endISO:eISO, ownerEmail:owner, ownerName, location:loc, type, caseId:caseId};
    DATA.calendar.push(ev); pushCalNotification('created', ev);
    App.set({}); return;
  }
  if(act==='deleteEvent'){
    const __ev=(DATA.calendar||[]).find(ev=>ev.id===arg); if(__ev){pushCalNotification('deleted', __ev);} DATA.calendar = (DATA.calendar||[]).filter(ev=>ev.id!==arg);
    App.set({}); return;
  }
  if(act==='openEvent'){
    const ev=(DATA.calendar||[]).find(x=>x.id===arg);
    if(!ev) return; App.state.modalEventId=ev.id; App.set({});
    setTimeout(()=>{
      const d=new Date(ev.startISO), end=new Date(ev.endISO);
      const pad=n=>String(n).padStart(2,'0');
      const ymd=d.toISOString().slice(0,10);
      const st=pad(d.getHours())+':'+pad(d.getMinutes());
      const et=pad(end.getHours())+':'+pad(end.getMinutes());
      const S=(id,v)=>{const el=document.getElementById(id); if(el) el.value=v;};
      S('md-title', ev.title||''); S('md-date', ymd); S('md-type', ev.type||'Appointment'); S('md-start', st); S('md-end', et); S('md-loc', ev.location||'');
      const csSel=document.getElementById('md-case'); if(csSel){ csSel.value=ev.caseId||''; }
      const own=document.getElementById('md-owner'); if(own){ own.value=ev.ownerEmail||''; }
    },0);
    return;
  }
  if(act==='closeModal'){ App.state.modalEventId=null; App.set({}); return; }
