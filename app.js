(function(){ "use strict";
const BUILD="baseline-1.3.0"; const STAMP=(new Date()).toISOString();
...
const App={state:{route:"dashboard",currentCaseId:null,currentCompanyId:null,currentContactId:null,
  ...
  settings:{emailAlerts:true, darkMode:false}, notifications:[], notificationsUnread:0, notificationsShowAll:false, modalEventId:null, calendar:(function(){return { view:"month", ym:(new Date()).toISOString().slice(0,7), selectedDate:(new Date()).toISOString().slice(0,10), filterUsers:"ALL" };})()}, set(p){Object.assign(App.state,p||{}); render();}, get(){return DATA;}};
...
}); const back=(me.role!=="Admin"?'<button class="btn light" data-act="clearImpersonation">Switch to Admin</button>':""); return `<div class="topbar"><div class="brand">Synergy CRM</div><div class="sp"></div><div class="muted" style="margin-right:10px">You: ${me.name||"Unknown"} (${me.role||"User"})</div>${back}<span class="badge">Soft Stable ${BUILD}</span></div>`; }
...
function Shell(content,active){ 
  const modal=`<div id="modal" class="modal" style="display:${App.state.modalEventId?'flex':'none'}">
    <div class="modal-card">
      ...
      <div class="grid cols-3">
        ...
        <div><label>Location</label><input class="input" id="md-loc"></div>
        <div><label>Case (optional)</label><select class="input" id="md-case"><option value="">— None —</option>${DATA.cases.map(cs=>`<option value="${cs.id}">${cs.fileNumber} — ${esc(cs.title)}</option>`).join("")}</select></div>
        ${(DATA.me&&DATA.me.role==='Admin')?`<div><label>Owner</label><select class="input" id="md-owner">${DATA.users.map(u=>`<option value="${u.email}">${u.name}</option>`).join("")}</select></div>`:''}
      </div>
      <div class="right" style="margin-top:8px"><button class="btn danger" data-act="modalDelete">Delete</button> <button class="btn light" data-act="closeModal">Cancel</button> <button class="btn" data-act="modalSave">Save</button></div>
    </div>
  </div>`;
  return Topbar()+`<div class="shell">${Sidebar(active)}<main class="main">${content}</main></div>${modal}<div id="boot">Ready (${BUILD})</div>`; 
}<main class="main">${content}</main></div><div id="boot">Ready (${BUILD})</div>`; }
...
}</td><td>${c.organisation}</td><td>${c.investigatorName}</td><td>${statusChip(c.status)}</td><td class="right"><button class="btn light" data-act="openCase" data-arg="${c.id}">Open</button></td></tr>`).join("");
...
function CasePage(id){
  ...
  const body = `<div class="tabpanel ${tab==='details'?'active':''}">${details}</div>
                ...
                <div class="tabpanel ${tab==='calendar'?'active':''}">${calTab}</div>`;
...
document.addEventListener('click', e=>{
  ...
  if(act==='closeModal'){ App.state.modalEventId=null; App.set({}); return; }
  if(act==='modalSave'){ const id=App.state.modalEventId; if(!id) return; const ev=(DATA.calendar||[]).find(x=>x.id===id); if(!ev) return;
    const G=id=>{const el=document.getElementById(id); return el?el.value:null;};
    const date=G('md-date')||ev.startISO.slice(0,10); const s=G('md-start')||'09:00'; const ed=G('md-end')||'10:00';
    ev.title=G('md-title')||ev.title; ev.type=G('md-type')||ev.type; ev.location=G('md-loc')||ev.location; ev.caseId=G('md-case')||'';
    const own=document.getElementById('md-owner'); if(own){ ev.ownerEmail=own.value; const u=DATA.users.find(u=>u.email===ev.ownerEmail)||{}; ev.ownerName=u.name||ev.ownerEmail; }
    ev.startISO=date+'T'+s+':00'; ev.endISO=date+'T'+ed+':00'; pushCalNotification('updated', ev);
    App.state.modalEventId=null; App.set({}); return; }
  if(act==='modalDelete'){ const id=App.state.modalEventId; if(!id) return; const ev=(DATA.calendar||[]).find(x=>x.id===id); if(ev) pushCalNotification('deleted', ev); DATA.calendar=(DATA.calendar||[]).filter(x=>x.id!==id); App.state.modalEventId=null; App.set({}); return; }
  ...
  if(act==='markNotifsRead'){ (App.state.notifications||[]).forEach(n=>n.read=true); App.state.notificationsUnread=0; App.set({}); return; }
  if(act==='readNotif'){ const id=arg; const n=(App.state.notifications||[]).find(x=>x.id===id); if(n){ n.read=true; App.state.notificationsUnread=(App.state.notifications.filter(x=>!x.read)).length; } App.set({}); return; }
  if(act==='openNotif'){ const n=(App.state.notifications||[]).find(x=>x.id===arg); if(!n){return;} const ev=(DATA.calendar||[]).find(x=>x.startISO===n.startISO && x.title===n.title); if(ev){ App.state.modalEventId=ev.id; App.set({route:'calendar'}); setTimeout(()=>{ const trigger=document.querySelector('[data-act="openEvent"][data-arg="'+ev.id+'"]'); if(trigger) trigger.click(); },0);} else { App.set({route:'calendar'}); } return; }
});
  ...
  const form = (()=>{
    ...
    return `<div class="card"><h3 class="section-title">Add ${isAdmin?"Event (any user)":"My Event"}</h3>
      <div class="grid cols-3">
        ...
        <div><label>Case (optional)</label><select class="input" id="ev-case"><option value="">— None —</option>${DATA.cases.map(cs=>`<option value="${cs.id}">${cs.fileNumber} — ${esc(cs.title)}</option>`).join("")}</select></div>
        ${isAdminOwner}
      </div>
...
function pushCalNotification(action, ev){
  ...
  App.state.notifications=[item].concat(App.state.notifications||[]).slice(0,100);
...
document.addEventListener('click', e=>{
  ...
  if(act==='createEvent'){
    ...
    const type=(document.getElementById('ev-type')||{}).value||'Meeting';
    const loc=(document.getElementById('ev-loc')||{}).value||'';
    const caseId=(document.getElementById('ev-case')||{}).value||'';
    const ownerEmail=(document.getElementById('ev-owner')||{}).value||me.email||'';
    const startISO = date+'T'+start+':00';
    const endISO = date+'T'+end+':00';
    const ev={ id:uid(), title, startISO, endISO, location:loc, caseId, ownerEmail, type };
    DATA.calendar = (DATA.calendar||[]).concat(ev);
    pushCalNotification('created', ev);
    App.set({}); return;
  }

  if(act==='openEvent' || act==='editEvent'){
    const id=t.getAttribute('data-arg'); if(!id) return;
    App.state.modalEventId=id; App.set({}); return;
  }
  ...
  if(act==='deleteEvent'){
    ...
    if(ix>-1){ const ev=list[ix]; list.splice(ix,1); pushCalNotification('deleted', ev); }
    DATA.calendar = list; App.set({}); return;
  }

  if(act==='modalClose'){ App.state.modalEventId=null; App.set({}); return; }

  if(act==='modalSave'){
    const id=App.state.modalEventId; if(!id) return;
    const ev=(DATA.calendar||[]).find(x=>x.id===id); if(!ev) return;
    const G=id=>{const el=document.getElementById(id); return el?el.value:null;};
    const date=G('md-date')||ev.startISO.slice(0,10);
    const s=G('md-start')||'09:00';
    const ed=G('md-end')||'10:00';
    ev.title=G('md-title')||ev.title;
    ev.startISO=date+'T'+s+':00';
    ev.endISO=date+'T'+ed+':00';
    ev.location=G('md-loc')||ev.location;
    ev.caseId=G('md-case')||ev.caseId;
    ev.ownerEmail=G('md-owner')||ev.ownerEmail;
    pushCalNotification('updated', ev);
    App.state.modalEventId=null; App.set({}); return;
  }
  ...
  if(act==='modalDelete'){
    ...
    const ix=list.findIndex(x=>x.id===id);
    if(ix>-1){ const ev=list[ix]; list.splice(ix,1); pushCalNotification('deleted', ev); }
    DATA.calendar = list; App.state.modalEventId=null; App.set({}); return;
  }
});
