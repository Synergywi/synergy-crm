...
function CasePage(id){
  const d=App.get ? App.get() : DATA;
  const cs = (typeof findCase==='function') ? findCase(id) : (d.cases||[]).find(x=>x.id===id);
  ...
  const details = '<div class="tab-content"><div class="card"><div class="grid cols-2" style="margin-top:4px">'
...
function ContactPage(id){
  const d=App.get ? App.get() : DATA;
  const c = (typeof findContact==='function') ? findContact(id) : (d.contacts||[]).find(x=>x.id===id);
  if(!c) return Shell('<div class="card">Contact not found.</div>','contacts');
  c.portal = c.portal || {enabled: !!(c.email), role: (c.role||'Investigator')};
  ...
  const details = '<div class="tab-content"><div class="grid cols-4" style="margin-top:12px">'
  ...
  const portal = '<div class="tab-content"><div class="card"><div class="grid cols-4">'
    +'<div><label>Status</label><div class="chip">'+(c.portal.enabled?'Enabled':'Disabled')+'</div></div>'
    +'<div><label>Role</label><select class="input" id="ct-role"><option'+(c.portal.role==='Client'?' selected':'')+'>Client</option><option'+(c.portal.role==='Investigator'?' selected':'')+'>Investigator</option><option'+(c.portal.role==='Reviewer'?' selected':'')+'>Reviewer</option><option'+(c.portal.role==='Admin'?' selected':'')+'>Admin</option></select></div>'
