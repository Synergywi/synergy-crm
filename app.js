(function(){ "use strict";
const BUILD="v2.12.2"; const STAMP=(new Date()).toISOString();
...
const App={state:{route:"companies",currentCaseId:null,currentContactId:null,currentCompanyId:"C-001",currentUploadTarget:null,currentCompanyUploadTarget:null,companyTab:"summary",caseTab:"details",contactTab:"details"}, set(p){Object.assign(App.state,p||{}); render();}, get(){return DATA;}};
...
function CasePage(id){
  ...
  const active=App.state.caseTab||"details";
  const tabBtns=tabs.map(t=>'<div class="tab '+(active===t[0]?'active':'')+'" data-act="caseTab" data-arg="'+t[0]+'">'+t[1]+'</div>').join('');

  const details = '<div class="card"><div class="grid cols-2" style="margin-top:4px">'
  ...
  const body = '<div class="card"><div class="tabs">'+tabBtns+'</div>'
              + (active==='notes'?notes:(active==='tasks'?tasks:(active==='docs'?docs:details)))
              + '</div>';
...
function ContactPage(id){
  const d=App.get(), c=findContact(id);
  if(!c) return Shell('<div class="card">Contact not found.</div>','contacts');
  if(!c.portal) c.portal = {enabled:true, role:(c.role||'Investigator')};
  ...
  const header = '<div class="card"><div style="display:flex;align-items:center;gap:8px"><h2>Contact</h2><div class="sp"></div><button class="btn" data-act="saveContact" data-arg="'+id+'">Save</button><button class="btn danger" data-act="deleteContact" data-arg="'+id+'">Delete</button><button class="btn light" data-act="route" data-arg="contacts">Back to Contacts</button></div></div>';

  const tabs=[["details","Details"],["portal","Portal"]];
  const active=App.state.contactTab||"details";
  const tabBtns=tabs.map(t=>'<div class="tab '+(active===t[0]?'active':'')+'" data-act="contactTab" data-arg="'+t[0]+'">'+t[1]+'</div>').join('');

  const details = '<div class="card"><div class="grid cols-4" style="margin-top:12px">'
  ...
  const portal = '<div class="card"><div class="grid cols-4">'
    +'<div><label>Status</label><div class="chip">'+(c.portal.enabled?'Enabled':'Disabled')+'</div></div>'
    +'<div><label>Role</label><select class="input" id="ct-role"><option'+(c.portal.role==='Investigator'?' selected':'')+'>Investigator</option><option'+(c.portal.role==='Reviewer'?' selected':'')+'>Reviewer</option><option'+(c.portal.role==='Admin'?' selected':'')+'>Admin</option></select></div>'
    +'<div class="right" style="grid-column:span 4;margin-top:8px"><button class="btn light" data-act="viewPortal" data-arg="'+id+'">View portal</button> <button class="btn light" data-act="updateRole" data-arg="'+id+'">Update Role</button> <button class="btn light" data-act="revokePortal" data-arg="'+id+'">Revoke</button></div>'
    +'</div></div>';

  const body = '<div class="card"><div class="tabs">'+tabBtns+'</div>'
              + (active==='portal'?portal:details)
              + '</div>';

  return Shell(header + body,'contacts');
}

// Companies list + page (kept from earlier build)
...
function CompanyProfile(co){
  const header = '<div class="card"><div style="display:flex;align-items:center;gap:10px"><div class="avatar">'+(co.name?co.name[0]:"?")+'</div><div><h2 style="margin:0">'+(co.name||'')+'</h2><div class="mono">'+(co.phone||'')+' · '+(co.email||'')+'</div></div><div class="sp"></div><button class="btn" data-act="editCompany" data-arg="'+co.id+'">Edit</button><button class="btn danger" data-act="deleteCompany" data-arg="'+co.id+'">Delete</button><button class="btn light" data-act="route" data-arg="companies">Back</button></div></div>';
  const about = '<div class="card profile"><h3 style="margin:0 0 8px">About this company</h3>'
    + '<div class="kvs">'
    +   '<div class="k">Industry</div><div>'+coVal(co,"industry")+'</div>'
    +   '<div class="k">Type</div><div>'+coVal(co,"type")+'</div>'
    +   '<div class="k">State</div><div>'+coVal(co,"state")+'</div>'
    +   '<div class="k">City</div><div>'+coVal(co,"city")+'</div>'
    +   '<div class="k">Postcode</div><div>'+coVal(co,"postcode")+'</div>'
    +   '<div class="k">ABN</div><div>'+coVal(co,"abn")+'</div>'
    +   '<div class="k">ACN</div><div>'+coVal(co,"acn")+'</div>'
    +   '<div class="k">Website</div><div>'+coVal(co,"website")+'</div>'
    + '</div></div>';
  return header + about;
}
...
function CompanyPage(id){
  ...
  const tabBtn = (key,label)=>'<div class="tab '+(tab===key?'active':'')+'" data-act="companyTab" data-arg="'+key+'">'+label+'</div>';

  // Tabs content
  let summary = '<div class="card"><h3 class="section-title">Recent Cases</h3>';
  const d=App.get(); const cases=d.cases.filter(c=>c.companyId===co.id).slice(0,6);
  if(!cases.length) summary += '<div class="muted">No cases for this company.</div>';
  else { summary += '<table><thead><tr><th>Case</th><th>Title</th><th>Status</th><th></th></tr></thead><tbody>'+cases.map(c=>'<tr><td>'+c.fileNumber+'</td><td>'+c.title+'</td><td>'+c.status+'</td><td class="right"><button class="btn light" data-act="openCase" data-arg="'+c.id+'">Open</button></td></tr>').join('')+'</tbody></table>'; }
  summary += '</div>';

  // Contacts tab
  const companyContacts = d.contacts.filter(c=>c.companyId===co.id);
  let cRows = companyContacts.length? '' : '<tr><td colspan="5" class="muted">No contacts yet.</td></tr>';
  for(const cc of companyContacts){
    const mail = cc.email?('<a class="btn light" href="mailto:'+cc.email+'" title="Email '+cc.name+'">Email</a>') : '';
    cRows += '<tr><td>'+cc.name+'</td><td>'+(cc.email||'')+'</td><td>'+(cc.phone||'')+'</td><td>'+(cc.org||'')+'</td><td class="right"><button class="btn light" data-act="openContact" data-arg="'+cc.id+'">Open</button> '+mail+'</td></tr>';
  }
  const contactsTab = '<div class="card"><div style="display:flex;align-items:center;gap:8px"><h3 class="section-title">Company Contacts</h3><div class="sp"></div><button class="btn" data-act="addCompanyContact" data-arg="'+co.id+'">Add contact</button></div><table><thead><tr><th>Name</th><th>Email</th><th>Phone</th><th>Position/Org</th><th></th></tr></thead><tbody>'+cRows+'</tbody></table></div>';
  ...
  for(const fname in co.folders){ if(!Object.prototype.hasOwnProperty.call(co.folders,fname)) continue; const files=co.folders[fname];
    docRows+='<tr><th colspan="3">'+fname+'</th></tr>';
    docRows+='<tr><td colspan="3" class="right"><button class="btn light" data-act="coSelectFiles" data-arg="'+co.id+'::'+fname+'">Upload to '+fname+'</button> '+(fname==='General'?'':'<button class="btn light" data-act="coDeleteFolder" data-arg="'+co.id+'::'+fname+'">Delete folder</button>')+'</td></tr>';
    if(!files.length){docRows+='<tr><td colspan="3" class="muted">No files</td></tr>';}
    for(const ff of files){const a=co.id+'::'+fname+'::'+ff.name; docRows+='<tr><td>'+ff.name+'</td><td>'+ff.size+'</td><td class="right">'+(ff.dataUrl?('<button class="btn light" data-act="coViewDoc" data-arg="'+a+'">View</button> '):'')+'<button class="btn light" data-act="coRemoveDoc" data-arg="'+a+'">Remove</button></td></tr>'; }
  }
  const docsTab = '<div class="card"><div style="display:flex;align-items:center;gap:8px"><h3 class="section-title">Company Documents</h3><div class="sp"></div><button class="btn light" data-act="coAddFolderPrompt" data-arg="'+co.id+'">Add folder</button></div><input type="file" id="co-file-input" multiple style="display:none"><table><thead><tr><th>File</th><th>Size</th><th></th></tr></thead><tbody>'+docRows+'</tbody></table></div>';

  const right = '<div><div class="tabs">'+tabBtn('summary','Summary')+tabBtn('contacts','Company Contacts')+tabBtn('docs','Company Documents')+'</div>'
    + (tab==='summary'?summary:(tab==='contacts'?contactsTab:docsTab)) + '</div>';

  return Shell(CompanyProfile(co) + '<div class="page-wrap">'+ right +'</div>', 'companies');
}

// Documents
function Documents(){return Shell('<div class="card">Global Documents</div>','documents');}

// Resources
function Resources(){return Shell('<div class="card">Resources</div>','resources');}

// Render
function render(){const r=App.state.route, el=document.getElementById('app'); document.getElementById('boot').textContent='Rendering '+r+'…'; if(r==='dashboard') el.innerHTML=Dashboard(); else if(r==='cases') el.innerHTML=Cases(); else if(r==='case') el.innerHTML=CasePage(App.state.currentCaseId); else if(r==='contacts') el.innerHTML=Contacts(); else if(r==='contact') el.innerHTML=ContactPage(App.state.currentContactId); else if(r==='companies') el.innerHTML=Companies(); else if(r==='company') el.innerHTML=CompanyPage(App.state.currentCompanyId); else if(r==='documents') el.innerHTML=Documents(); else if(r==='resources') el.innerHTML=Resources(); else if(r==='admin') el.innerHTML='<div class="card">Admin</div>'; else el.innerHTML=Dashboard(); document.getElementById('boot').textContent='Ready ('+BUILD+')';}
...
// nav
if(act==='route'){const go=arg; const upd={route:go}; if(go==='case'){upd.caseTab='details';} if(go==='contact'){upd.contactTab='details';} if(go==='company'){upd.companyTab='summary';} App.set(upd); return;}
if(act==='openCase'){App.set({currentCaseId:arg,route:'case',caseTab:'details'});return;}
if(act==='openContact'){App.set({currentContactId:arg,route:'contact',contactTab:'details'});return;}
if(act==='openCompany'){App.set({currentCompanyId:arg,route:'company',companyTab:'summary'});return;}
...
// contacts
if(act==='newContact'){const c={id:uid(),name:'New Contact',email:'',phone:'',org:'',companyId:'',notes:''}; DATA.contacts.unshift(c); App.set({currentContactId:c.id,route:'contact',contactTab:'details'}); return;}
...
if(act==='updateRole'){const c=findContact(arg); if(!c) return; const el=document.getElementById('ct-role'); if(el){ c.portal=c.portal||{}; c.portal.role=el.value; alert('Role updated to '+c.portal.role); } return;}
...
if(act==='viewPortal'){alert('Opening portal (stub)'); return;}

// companies
if(act==='openCompany'){App.set({currentCompanyId:arg,route:'company',companyTab:'summary'});return;}
if(act==='newCompany'){const co={id:'C-'+('00'+(DATA.companies.length+1)).slice(-3),name:'New Company',folders:{General:[]}}; DATA.companies.unshift(co); App.set({currentCompanyId:co.id,route:'company',companyTab:'summary'}); return;}
if(act==='editCompany'){ const co=findCompany(arg); if(!co) return; const f=`<div class="grid cols-2" style="margin-top:8px">
  ...
  const wrap=document.createElement('div'); wrap.innerHTML='<div class="card"><div style="display:flex;align-items:center;gap:8px"><h3>Edit Company</h3><div class="sp"></div><button class="btn light" data-act="__closeDlg">Close</button></div>'+f+'<div class="right" style="margin-top:8px"><button class="btn" data-act="saveCompany" data-arg="'+arg+'">Save</button></div></div>';
...
if(act==='__closeDlg'){ const dlg=document.getElementById('dlg'); if(dlg) dlg.remove(); return; }
if(act==='saveCompany'){ const co=findCompany(arg); if(!co) return;
  const get=(id)=>{const el=document.getElementById(id); return el?el.value:''; };
  co.name=get('co-name'); co.industry=get('co-industry'); co.type=get('co-type'); co.phone=get('co-phone'); co.email=get('co-email'); co.website=get('co-website'); co.abn=get('co-abn'); co.acn=get('co-acn'); co.state=get('co-state'); co.city=get('co-city'); co.postcode=get('co-postcode');
  const dlg=document.getElementById('dlg'); if(dlg) dlg.remove(); App.set({}); return;
}
...
if(act==='addCompanyContact'){ const nc={id:uid(),name:'New Contact',email:'',phone:'',org:'',companyId:arg,notes:''}; DATA.contacts.unshift(nc); App.set({currentContactId:nc.id,route:'contact',contactTab:'details'}); return; }
...
if(act==='coSelectFiles'){App.state.currentCompanyUploadTarget=arg||((App.state.currentCompanyId||'')+'::General'); const fi=document.getElementById('co-file-input'); if(fi) fi.click(); return;}
...
document.addEventListener('change',e=>{
  const t=e.target;
  if(!t) return;
  if(t.id==='flt-q'){const f=App.state.casesFilter||{q:""}; f.q=t.value; App.state.casesFilter=f; try{localStorage.setItem('synergy_filters_cases_v2122', JSON.stringify(f));}catch(_){ } App.set({});}
  if(t.id==='file-input'){ const p=(App.state.currentUploadTarget||'').split('::'); const cs=findCase(p[0]); if(!cs) return; const folder=p[1]||'General'; cs.folders[folder]=cs.folders[folder]||[]; const files=t.files||[]; const rd=(file)=>new Promise(res=>{const r=new FileReader(); r.onload=()=>res({name:file.name,size:file.size,dataUrl:r.result}); r.readAsDataURL(file);}); (async()=>{for(let i=0;i<files.length;i++){cs.folders[folder].push(await rd(files[i]));} App.set({caseTab:'docs'});})(); }
  if(t.id==='co-file-input'){ const p=(App.state.currentCompanyUploadTarget||'').split('::'); const co=findCompany(p[0]); if(!co) return; const folder=p[1]||'General'; co.folders[folder]=co.folders[folder]||[]; const files=t.files||[]; const rd=(file)=>new Promise(res=>{const r=new FileReader(); r.onload=()=>res({name:file.name,size:file.size,dataUrl:r.result}); r.readAsDataURL(file);}); (async()=>{for(let i=0;i<files.length;i++){co.folders[folder].push(await rd(files[i]));} App.set({companyTab:'docs'});})(); }
});
