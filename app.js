...
(function(){
  try {
    ...
    if(anyEmail){ const _k='mail:'+normEmail(anyEmail.email); if(!DATA.userSessions[_k]){
      var now = new Date();
      var arr = DATA.userSessions[_k] = [];
...
(function(){
  try{
    ...
    emails.forEach(function(em, idx){
      var arr = DATA.userSessions['mail:'+normEmail(em)] = DATA.userSessions['mail:'+normEmail(em)] || [];
      function add(daysAgo, mins){ var d=new Date(now.getTime()-daysAgo*24*60*60*1000); arr.push({ start: d.toISOString(), durationMins: mins }); }
      add(1+idx, 45+idx*5);
      add(8+idx, 12+idx*7);
      add(30+idx, 78+idx*3);
...
function CasePage(id){const d=App.get(), cs=findCase(id); if(!cs) 
...
const header = '<div class="card">'
 ...
 +   '<div class="sp"></div>'
 +   '<button class="btn" data-act="saveCase" data-arg="'+id+'">Save</button>'
 +   ' <button class="btn light" data-act="addQuickNote" data-arg="'+id+'">Add Note</button>'
 +   ' <button class="btn light" data-act="selectFiles" data-arg="'+id+'::General">Upload</button>'
 +   ' <button class="btn light" data-act="route" data-arg="cases">Back to Cases</button>'
 + '</div>'
 + '</div>';

const leftDetails = '<div class="card">'
 + '<div class="grid cols-1" style="margin-top:4px">'
 +   '<div><label>Title</label><input class="input" id="c-title" value="'+(cs.title||'')+'"></div>'
 +   '<div><label>Organisation</label><input class="input" id="c-org" value="'+(cs.organisation||'')+'"></div>'
...
let notesList = (cs.notes&&cs.notes.length) ? ('<ul style="margin:0 0 0 16px;padding:0">'+cs.notes.map(nn=>'<li>'+ (nn.text||'') +'</li>').join('') + '</ul>') : '<ul style="margin:0 0 0 16px;padding:0"><li class="muted">No notes yet</li></ul>';
const rightNotes = '<div class="card">'
 +   '<strong>Notes</strong>'
 +   '<div style="margin-top:8px">'+notesList+'</div>'
 + '</div>';

const twoCol = '<div class="grid cols-2">'+leftDetails+rightNotes+'</div>';

// Keep documents section below (unchanged)
let docRows='';
...
let html='<div class="card"><div style="display:flex;align-items:center;gap:8px"><h2>Contact</h2><div class="sp"></div><button class="btn" data-act="saveContact" data-arg="'+id+'">Save</button><button class="btn light" data-act="route" data-arg="contacts">Back to Contacts</button></div><div class="grid cols-4" style="margin-top:12px"><div><label>Contact Name</label><input class="input" id="ct-name" value="'+(c.name||'')+'"></div><div><label>Email</label><input class="input" id="ct-email" value="'+(c.email||'')+'"></div><div><label>Phone</label><input class="input" id="ct-phone" value="'+(c.phone||'')+'"></div><div><label>Position/Org</label><input class="input" id="ct-org" value="'+(c.org||'')+'"></div><div style="grid-column:span 2"><label>Link to Company</label><select class="input" id="ct-company">'+coOpts()+'</select></div><div style="grid-column:span 4"><label>Notes</label><textarea class="input" id="ct-notes">'+(c.notes||'')+'</textarea></div></div></div><div class="section"><header><h3 class="section-title">Portal Access</h3><div>'+headerBtns+'</div></header>'+ portalBody +'</div>'; // --- Inject "Last seen" badge next to the name (in the <h2>) ---
  (function(){
    try {
      ...
      var key = sessionKeyForContact(c);
      var sessions = (window.DATA && DATA.userSessions && key) ? (DATA.userSessions[key] || []) : [];
...
if(act==='simulateLogin'){ const c=findContact(arg); if(!c){ alert('Contact not found'); return; }
  DATA.userSessions = DATA.userSessions || {};
  const key = sessionKeyForContact(c);
  if(!key){ alert('Add an email or save contact first'); return; }
  const arr = DATA.userSessions[key] = DATA.userSessions[key] || [];
  const now = new Date();
...
if(act==='clearUserLog'){ const c=findContact(arg); if(!c){ return; }
  DATA.userSessions = DATA.userSessions || {};
  const key = sessionKeyForContact(c);
  if(!key){ return; }
  DATA.userSessions[key] = [];
  saveUserSessions();
  App.set({}); return;
}
...
document.addEventListener('change',e=>{
  if(e.target && e.target.id==='snapshot-input'){const files=[...(e.target.files||[])]; if(!files.length) return; const r=new FileReader(); r.onload=ev=>{try{const incoming = JSON.parse(ev.target.result);
      const sess = incoming && incoming.userSessions;
      if(sess){
        DATA.userSessions = DATA.userSessions || {};
        Object.keys(sess).forEach(k=>{ const nk = (k.indexOf('mail:')===0||k.indexOf('contact:')===0)?k:('mail:'+normEmail(k)); DATA.userSessions[nk]=sess[k]; });
        saveUserSessions();
      }
      Object.assign(DATA, incoming);
      App.set({});}catch(err){alert('Invalid JSON: '+err.message);}}; r.readAsText(files[0]);}
