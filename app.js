(function(){ "use strict";
const BUILD="v2.12.0"; const STAMP=(new Date()).toISOString();
...
const App={state:{route:"dashboard",currentCaseId:null,currentContactId:null,currentCompanyId:null,currentUploadTarget:null,currentCompanyUploadTarget:null,companyTab:"summary",companyEdit:false,casesFilter:{q:""}}, set(p){Object.assign(App.state,p||{}); render();}, get(){return DATA;}};
...
function CasePage(id){
  ...
  return Shell(header + details + lower + docs, 'cases');
}
...
// Company page (Tabbed, left profile)
function CompanyPage(id){
  ...
  const left = '<div class="profile-card">'
    + '<div class="profile-head"><div class="avatar">'+(co.name?co.name[0]:"")+'</div><div><div style="font-weight:700">'+(co.name||"")+'</div><div class="meta-row"><small>'+ (co.email||"") +'</small> · <small>'+ (co.phone||"") +'</small></div></div><div class="sp"></div><button class="btn light" data-act="editCompany" data-arg="'+co.id+'">Edit</button></div>'
    + '<div class="profile-meta">'
    +   '<div class="meta-row"><small>Website</small> '+(co.website||'—')+'</div>'
    +   '<div class="meta-row"><small>Industry</small> '+(co.industry||'—')+'</div>'
    +   '<div class="meta-row"><small>Type</small> '+(co.type||'—')+'</div>'
    +   '<div class="meta-row"><small>ABN</small> '+(co.abn||'—')+'</div>'
    +   '<div class="meta-row"><small>ACN</small> '+(co.acn||'—')+'</div>'
    +   '<div class="meta-row"><small>Location</small> '+[(co.city||''),(co.state||''),(co.postcode||'')].filter(Boolean).join(', ')+'</div>'
    + '</div>'
    + '</div>';

  // Tabs
  const tabs = [["summary","Summary"],["contacts","Company Contacts"],["docs","Company Documents"]];
  const active=App.state.companyTab||"summary";
  let tabButtons = tabs.map(t=>'<div class="tab '+(active===t[0]?'active':'')+'" data-act="companyTab" data-arg="'+t[0]+'">'+t[1]+'</div>').join('');

  // Summary content
  const summary = '<div class="tab-content"><div class="grid cols-2">'
    + '<div class="card"><div class="section"><header><h3 class="section-title">Overview</h3></header>'
    ...
    + '</div></div></div>'
    + '<div class="card"><div class="section"><header><h3 class="section-title">Recent Cases</h3></header>'
    + '<table><thead><tr><th>Case</th><th>Title</th><th>Status</th><th></th></tr></thead><tbody>'
    + (DATA.cases.filter(cs=>cs.companyId===co.id).map(cs=>'<tr><td>'+cs.fileNumber+'</td><td>'+cs.title+'</td><td><span class="pill">'+cs.status+'</span></td><td class="right"><button class="btn light" data-act="openCase" data-arg="'+cs.id+'">Open</button></td></tr>').join('')||'<tr><td colspan="4">No cases</td></tr>')
    + '</tbody></table></div></div>'
    + '</div></div>';

  // Contacts content
  const contacts = (DATA.contacts||[]).filter(c=>c.companyId===co.id);
  let cRows=''; if(!contacts.length){ cRows='<tr><td colspan="5">No contacts yet.</td></tr>';}
  for(const c of contacts){ cRows+='<tr><td>'+c.name+'</td><td>'+c.email+'</td><td>'+ (c.org||"") +'</td><td>'+ (c.phone||"") +'</td><td class="right"><button class="btn light" data-act="openContact" data-arg="'+c.id+'">Open</button> '+(c.email?'<button class="btn light" data-act="emailContact" data-arg="'+c.id+'">Email</button>':'')+'</td></tr>'; }
  const contactsView = '<div class="tab-content"><div class="section"><header><h3 class="section-title">Company Contacts</h3><button class="btn" data-act="newContactForCompany" data-arg="'+co.id+'">Add contact</button></header><table><thead><tr><th>Name</th><th>Email</th><th>Position/Org</th><th>Phone</th><th></th></tr></thead><tbody>'+cRows+'</tbody></table></div></div>';
...
// tabs & edit
if(act==='companyTab'){App.set({companyTab:arg,companyEdit:false}); return;}
...
document.addEventListener('DOMContentLoaded',()=>{ App.set({route:'companies'});});
})();