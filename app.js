...
function pushNotif(payload){
  if(!DATA.notifications) DATA.notifications={items:[],showUnreadOnly:false};
  const it=Object.assign({ id: uid(), read:false, when:new Date().toISOString() }, payload||{});
  DATA.notifications.items.unshift(it);
}
...
const DATA={
  notifications:{ items:[], showUnreadOnly:false },
...
function Topbar(){
  const me=(DATA.me||{});
  const back=(me.role!=='Admin'&&App.state.asUser)?'<button class="btn light" data-act="exitPortal">Exit</button> ':'';
  let unread=0;
  if (DATA.me && DATA.me.role==='Admin' && DATA.notifications && DATA.notifications.items){
    unread = DATA.notifications.items.filter(n=>!n.read).length;
  }
  return `<div class="topbar">
    <div class="brand">Synergy CRM</div>
    <div class="sp"></div>
    ${ (DATA.me&&DATA.me.role==='Admin')
        ? `<button class="btn icon" title="Notifications" data-act="openNotifs">🔔${unread?'<span class="badge dot">'+unread+'</span>':''}</button>`
        : '' }
    ${back}<span class="badge">Soft Stable ${BUILD}</span>
  </div>`;
}
...
function Dashboard(){
  ...
  const overview=`<div class="card"><h3>Welcome</h3><div class="muted">${STAMP}</div></div><div class="section"><header><h3 class="section-title">Active Cases</h3></header><table><thead><tr><th>Case ID</th><th>Company</th><th>Investigator</th><th>Status</th><th></th></tr></thead><tbody>${rows}</tbody></table></div>` + NotifSection();
...
document.addEventListener('click', e=>{
  ...
  if(act==='route'){ App.set({route:arg
  if(act==='openNotifs'){ App.set({route:'dashboard'}); return; }
  if(act==='notifToggle'){ DATA.notifications.showUnreadOnly=!DATA.notifications.showUnreadOnly; App.set({}); return; }
  if(act==='notifMarkAll'){ (DATA.notifications.items||[]).forEach(n=>n.read=true); App.set({}); return; }
  if(act==='notifDismiss'){ const id=arg; (DATA.notifications.items||[]).forEach(n=>{ if(n.id===id) n.read=true; }); App.set({}); return; }
  if(act==='notifOpen'){ const id=arg; const n=(DATA.notifications.items||[]).find(x=>x.id===id); if(n){ n.read=true; } App.set({route:'calendar'}); return; }
}); return; }
...
function NotifSection(){
  if(!(DATA.me&&DATA.me.role==='Admin')) return '';
  const d=DATA.notifications||{items:[],showUnreadOnly:false};
  const items = d.items||[];
  const list = d.showUnreadOnly ? items.filter(n=>!n.read) : items;
  let body = '<div class="muted">No notifications</div>';
  if(list.length){
    body = list.map(n=>{
      const when=(n.when||'').replace('T',' ').slice(0,16);
      const ttl=n.title||''; const act=n.action||'';
      return `<div class="row" style="display:flex;gap:12px;align-items:center;justify-content:space-between;margin:6px 0">
        <div style="min-width:180px" class="mono">${when}</div>
        <div style="flex:1">${n.read?'<span class="muted">':''}${act} — ${ttl}${n.read?'</span>':''}</div>
        <div class="right">
          ${n.eventId?`<button class="btn light" data-act="notifOpen" data-arg="${n.id}">Open</button> `:''}
  ...
  const hdrBtns = `<div>
    <button class="btn light" data-act="notifToggle">${d.showUnreadOnly?'Show all':'Show unread'}</button>
    <button class="btn light" data-act="notifMarkAll">Mark all read</button>
  </div>`;
  return `<div class="section">
    <header><h3 class="section-title">Calendar updates (Admin)</h3>${hdrBtns}</header>
    ${body}
  </div>`;
}
