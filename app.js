/* Synergy CRM v2136 — Calendar + Case linkage + Notifications */
(function(){
  "use strict";
  const BUILD = "v2136"; const STAMP=(new Date()).toISOString();
  const $ = (s, r=document)=>r.querySelector(s);
  const $$ = (s, r=document)=>Array.from(r.querySelectorAll(s));
  const esc = s => (s==null?'':String(s)).replace(/[&<>"']/g, m=>({"&":"&amp;","<":"&lt;","»":"&raquo;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[m]));
  const uid = ()=> Math.random().toString(36).slice(2,10);
  const ymd = d => new Date(d).toISOString().slice(0,10);
  const hhmm = d => new Date(d).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'});

  // Data + persistence
  const STORE="SYNERGY_DATA_2136";
  const DATA = JSON.parse(localStorage.getItem(STORE)||"null") || {
    ...
    cases:[
      {id:'c1', fileNumber:'F-001', title:'Payroll Irregularities', organisation:'Acme Pty Ltd', companyId:'co1', investigatorEmail:'ivy@example.com', status:'Investigation', priority:'High', notes:[], tasks:[], folders:{General:[]}},
      {id:'c2', fileNumber:'F-002', title:'Supplier Collusion', organisation:'Globex Pty Ltd', companyId:'co2', investigatorEmail:'ivan@example.com', status:'Planning', priority:'Medium', notes:[], tasks:[], folders:{General:[]}},
    ],
    calendar:[
  ...
  // App state
  const App = { state:{ route:'dashboard', currentCaseId:null, tabs:{}, ownerFilter:'all', showUnread:false }, set(p){ Object.assign(App.state,p||{}); render(); } };

  // Shell
  function renderNav(){
    const items = ['dashboard','calendar','cases','companies','contacts','resources','admin'];
  ...
  function headerUser(){
    const me=DATA.me||{}; $('#userbox').innerHTML = `You: ${esc(me.name)} (${esc(me.role)})`;
  }
  ...
  function Dashboard(){
    ...
    const list = (App.state.showUnread? unread : (DATA.notifications||[])).slice(0,20);
    const rows = list.map(n=>`
      <tr>
        <td>${new Date(n.when).toLocaleString()}</td>
        <td>${esc(n.action)}</td>
        <td>${esc(n.title)}</td>
        <td class="right">
          <button class="btn" data-act="notifOpen" data-arg="${n.id}">Open</button>
      ...
      <div class="grid cols-2">
        ...
        <div class="card">
          ...
          <div class="right" style="margin-top:6px">
            <button class="btn light" data-act="notifToggle">${App.state.showUnread?'Show all':'Show unread'}</button>
            <button class="btn light" data-act="notifMarkAll">Mark all read</button>
          </div>
        </div>
        <div class="card">
          <h3 class="section-title">Active Cases</h3>
          <table><thead><tr><th>ID</th><th>Title</th><th>Organisation</th><th>Status</th><th></th></tr></thead><tbody>${cases}</tbody></table>
        </div>
      </div>
    `);
  }
  ...
  function Calendar(){
    ...
    const events = (DATA.calendar||[]).filter(e => ownerFilter==='all' || e.ownerEmail===ownerFilter)
  ...
  // Case page with Calendar section
  function CasePage(id){
    ...
    const tabsHtml = `<div class="tabs">
      ${tabs.map(k=>`<div class="tab ${tab===k?'active':''}" data-act="tab" data-scope="case" data-arg="${k}">${k[0].toUpperCase()+k.slice(1)}</div>`).join('')}
    </div>`;
  ...
  const Resources = () => `<div class="card"><h3 class="section-title">Resources</h3><div class="muted">Static</div></div>`;
  const Admin = () => `<div class="card"><h3 class="section-title">Admin</h3><div class="muted">Users (demo)</div></div>`;
  ...
  document.addEventListener('click', (e)=>{
    ...
    if(act==='notifToggle'){ App.set({showUnread:!App.state.showUnread}); return; }
    if(act==='notifMarkAll'){ (DATA.notifications||[]).forEach(n=>n.read=true); save(); App.set({}); return; }
    if(act==='notifOpen'){ const n=(DATA.notifications||[]).find(x=>x.id===arg); if(n){ n.read=true; const ev=DATA.calendar.find(e=>e.id===n.evId); if(ev) renderEventModal(ev);} save(); return; }
    if(act==='notifDismiss'){ DATA.notifications = (DATA.notifications||[]).filter(n=>n.id!==arg); save(); App.set({}); return; }
    ...
    if(act==='createEvent'){
      ...
      DATA.calendar.push(ev); notify('created', ev); save(); App.set({}); return;
    }
    ...
    if(act==='saveEvent'){
      ...
      notify('updated', ev); save(); closeModal(); App.set({}); return;
    }
    if(act==='deleteEvent'){ const ev=(DATA.calendar||[]).find(x=>x.id===arg); if(ev) notify('deleted', ev); DATA.calendar = (DATA.calendar||[]).filter(x=>x.id!==arg); save(); closeModal(); App.set({}); return; }
  ...
  document.addEventListener('change', (e)=>{
    if(e.target && e.target.id==='owner-filter'){ App.set({ownerFilter:e.target.value}); }
  });
