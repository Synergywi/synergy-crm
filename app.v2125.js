...
/*! Synergy CRM Calendar Hotfix v2125.4 */
(function(){
  if(window.__CAL_PATCH__) return;  // avoid double install
  window.__CAL_PATCH__ = "v2125.4";
  console.log("[Calendar] Hotfix", window.__CAL_PATCH__, "loaded");
  ...
  function renderEventModal(ev, opts){
    ...
    el.querySelector('[data-save]').addEventListener('click', ()=>{
      ...
      saveCal(); if(window.App&&App.set) App.set({}); close();
    });
    const del=el.querySelector('[data-del]');
    if(del){ del.addEventListener('click', ()=>{ const DATA=window.DATA||{}; DATA.calendar=(DATA.calendar||[]).filter(x=>x!==ev && x.id!==ev.id); saveCal(); if(window.App&&App.set) App.set({}); close(); }); }
  ...
  document.addEventListener('click', function(e){
    ...
    saveCal(); if(window.App&&App.set) App.set({});
  }, true);
})();
